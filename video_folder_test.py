...
def make_dataset(dir, nframes, class_to_idx):
	...
	for target in sorted(os.listdir(dir)):
		if os.path.isdir(os.path.join(dir,target))==True:
			# if  is a subfolder
			subfolder_path = os.path.join(dir, target) # eg: dir + '/rM7aPu9WV2Q'
			for subsubfold in sorted(os.listdir(subfolder_path) ):
				if os.path.isdir(os.path.join(subfolder_path, subsubfold) ):
					subsubfolder_path = os.path.join(subfolder_path, subsubfold) # eg: dir + '/rM7aPu9WV2Q/1'
					item_frames = []
					i = 1
					for fi in sorted( os.listdir(subsubfolder_path) ):
						...
						# fi is an image in the subsubfolder
							file_name = fi
							file_path = os.path.join(subsubfolder_path,file_name) # eg: dir + '/rM7aPu9WV2Q/1/rM7aPu9WV2Q_frames_00086552.jpg'
...
class VideoFolder(data.Dataset):
	...
	def __getitem__(self, index):
		...
		"""
		clip = self.imgs[index] # clip is a list of 32 frames 
		#print(len(clip))
		img_clip = []
		i = 0
		for frame in clip:
			...
			if self.transform is not None:
				img = self.transform(img) # ToTensor transforms the img to a FloatTensor, size 3*64 *64
